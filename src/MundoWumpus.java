...
public class MundoWumpus {
	...
	boolean mundoVisivel[][]; // linha, coluna, vetor de características do mundo Wumpus visível para o
								// agente, incluindo o próprio agente
	String mundoTela[][]; // linha, coluna, vetor de características do mundo Wumpus no formato de
							// impressão
	...
	public MundoWumpus() {
		...
		mundoVisivel = new boolean[][] { { false, false, false, false }, { false, false, false, false },
	...
	public void atualizaMundoTela(boolean ouro) {
		int i, j;
		for (i = 0; i < 4; i++)
			for (j = 0; j < 4; j++)
				mundoTela[i][j] = "";
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				...
				if (mundo[i][j][6]) // se tem agente
					mundoTela[i][j] += 'A';
				if (mundo[i][j][6] && mundo[i][j][5] && mundo[i][j][2]) {
					mundo[i][j][5] = false;
					mundo[i][j][2] = false;
	...
	public void mostraMundo(boolean ouro) {
		int i, j;
		atualizaMundoTela(ouro);
		// escrevendo de modo que a sala [0][0] seja a primeira de baixo para cima, da
		// esq para a dir
	...
	public void atualizarAgente(int iAtual, int jAtual, int iNovo, int jNovo, boolean percep) {
		mundo[iNovo][jNovo][6] = true;
		mundo[iAtual][jAtual][6] = false;
		mundo[iAtual][jAtual][5] = false;
		mundo[iAtual][jAtual][2] = false;
		if (percep == true) {

			mundo[iNovo][jNovo][5] = true;

			mundo[iNovo][jNovo][2] = true;

			this.atualizaMundoTela(percep);

		}
		// System.out.println("posI: "+iAtual);
	}
